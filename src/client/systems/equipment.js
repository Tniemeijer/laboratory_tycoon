...
// What a run of `n` samples of this step is missing from the shelf, or null if it can go ahead.
// Nothing is consumed here — it's asked before a worker is sent, and again before the run starts.
//
// Running short used to be survivable: the batch went ahead slower and the samples came out at
// reduced quality. That made the stock economy optional, which is the opposite of the point —
// you could simply never order slides and eat a quality hit forever. Work now stops dead until
...
export function runShortage(cap, proto, n) {
    ...
    if (step && step.reagent) {
        const have = s.reagents.filter(r => r.type === step.reagent && r.expire > s.day).length;
        if (have < n) return REAGENTS[step.reagent].name;
    }
    return null;
}
// Warns once per item per day rather than every tick. The check runs constantly while a batch
// sits blocked.
function warnShortage(st, missing) {
    const s = G.state;
...
export function batchReady(st) {
    ...
    for (const list of groups.values()) {
        ...
        if (list.length < batch && oldest < BATCH_MAX_WAIT) continue;
        // Don't offer a run that can't be finished: without the consumables it needs, the batch
        // stays staged and the machine idles until stock turns up.
        const missing = runShortage(list[0].cap, list[0].proto, Math.min(batch, list.length));
        if (missing) { warnShortage(st, missing); continue; }
        return list;
    }
    return null;
}
...
export function startRun(st, group) {
    ...
    // Re-checked here as well as in batchReady(): stock can be spent by another machine between
    // the worker being sent and them arriving. Checked *before* the samples are pulled out of
    // staging, so a refused run leaves them exactly where they were.
    const missing = runShortage(picked[0].cap, picked[0].proto, picked.length);
    if (missing) { warnShortage(st, missing); return null; }
    ...
    // Consumables for this run. Guaranteed present by the runShortage() check above. Per-sample
    ...
    for (const sm of samples) {
        if (step.reagent) takeReagent(step.reagent);     // availability already established
